...
function AdminOrderComponents() {
    ...
    const handleSortChange = (field) => {   // 정렬 기준 변경 함수
        const direction = field === sortOrder && sortDirection === 'asc' ? 'desc' : 'asc';  // 현재 정렬 기준이 같으면 방향 변경
    ...
    const downloadOrderDetails = () => {       // 주문 세부정보를 PDF로 다운로드하는 함수
    ...
    return (
        <div className='dashboard items-center p-8 bg-gray-100 w-full'>
            ...
            <div className={`grid ${widgetCol} gap-4 p-4 bg-gray-100`}>
                ...
                <Widget title="완료된 주문" value={orders.filter(order => order.status === '완료').length} />

                {/* filter 메서드를 사용하여 상태가 '취소'인 주문만 필터링하고, 그 길이를 계산하여 표시 */}
                <Widget title="취소된 주문" value={orders.filter(order => order.status === '취소').length} />
                ...
                <table className="min-w-full">
                    <thead>
                        <tr className="border-b">
                            ...
                            <th onClick={() => handleSortChange('orderNumber')} className="px-4 py-2 cursor-pointer">주문 번호</th>
                            <th onClick={() => handleSortChange('customerName')} className="px-4 py-2 cursor-pointer">고객 이름</th>
                            <th onClick={() => handleSortChange('status')} className="px-4 py-2 cursor-pointer">상태</th>
                            <th onClick={() => handleSortChange('orderDate')} className="px-4 py-2 cursor-pointer">주문 날짜</th>
                            <th onClick={() => handleSortChange('totalAmount')} className="px-4 py-2 cursor-pointer">총 금액</th>
                    ...
                    <tbody>
                        {filteredOrders.map(order => (
                            ...
                            <tr key={order.orderNumber} className="border-b">
                                ...
                                <td className="px-4 py-2 text-center">{order.totalAmount.toLocaleString()}원</td>
            ...
            {/* 보기 버튼 눌렀을 때 주문 세부정보 */}
            <div ref={orderDetailsRef} className="p-4 bg-white shadow-md rounded-lg mt-4">
                ...
                {selectedOrder ? (
                    <>
                        ...
                        <p><strong>주문 번호:</strong> {selectedOrder.orderNumber}</p>

                        {/* 주문한 항목 배열을 쉼표로 구분하여 문자열로 변환 */}
                        <p><strong>주문한 항목:</strong> {selectedOrder.items.join(', ')}</p>

                        <p><strong>배송 정보:</strong> {selectedOrder.deliveryInfo}</p>
                        <p><strong>고객 연락처:</strong> 010-1234-5678</p>
                        <p><strong>주문 상태:</strong> {selectedOrder.status}</p>
                        <p><strong>총 금액:</strong> {selectedOrder.totalAmount}</p>

                        {/* 추적 번호가 없을 경우 '정보 없음'으로 대체하여 표시 */}
                        <p><strong>추적 번호:</strong> {selectedOrder.trackingNumber || '정보 없음'}</p>

                        <p><strong>배송 상태:</strong> {selectedOrder.shippingStatus}</p>

                        {/* 버튼 클릭 시 downloadOrderDetails 함수를 호출하여 주문 세부정보를 PDF로 다운로드 */}
                        <button className="mt-2 bg-blue-500 text-white px-4 py-2 rounded" onClick={downloadOrderDetails}>PDF로 다운로드</button>
