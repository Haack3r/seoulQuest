...
//    // 카테고리별 상품 조회
//    @Query("select p, pi from Product p left join p.productImageList pi " +
//            "where pi.ord = 0 and p.delFlag = false and p.category.categoryName = :categoryName")
//    Page<Object[]> findByCategory(@Param("categoryName") String categoryName, Pageable pageable);
//
//    // 가격 범위로 상품 검색
//    @Query("select p, pi from Product p left join p.productImageList pi " +
//            "where pi.ord = 0 and p.delFlag = false " +
...
//
//    // 재고 부족 상품 조회 (재고관리용)
//    @Query("select p from Product p where p.delFlag = false and p.pqty <= :threshold")
...
//    Page<Object[]> findByPriceLessThanEqual(@Param("maxPrice") int maxPrice, Pageable pageable);
//
//    // 카테고리별 평균 가격 조회
