...
public class TourRepositoryTests {
    ...
    public void testInsert() {
        ...
        // Array of region strings, with each pair representing a region group.
        String[][] regionPairs = {
                {"Seoul", "Incheon"},
                {"Busan", "Daegu"},
                {"Gyeonggi", "Gangwon"},
                {"Jeju", "Jeolla"},
                {"Gyeongsang", "Chungcheong"}
        };

        for (int i = 0; i < tourDescriptions.length; i++) {
            // Choose a region based on the index, cycling through the array every 5 items.
            String[] selectedRegions = regionPairs[i / 5];

            Tour tour = Tour.builder()
                    .tname("투어" + (i + 1))
                    .tdesc(tourDescriptions[i] + " Region: " + selectedRegions[0] + ", " + selectedRegions[1])
                    .tprice(100 * (i + 1))
                    .maxCapacity(2 * (i + 1))
                    .tlocation("투어장소" + (i + 1))
                    .category(categoryRepository.getReferenceById(20L))
                    .area(selectedRegions[0] + ", " + selectedRegions[1]) // Add area string
                    .build();
...
//    }
    ...
    @Transactional
    @Test
    public void testTourMap(){
        List<Tour> list = tourRepository.findAllByArea("Seoul, Incheon");
