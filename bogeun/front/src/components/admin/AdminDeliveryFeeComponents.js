...
const AdminDeliveryFeeComponents = () => {
  ...
  useEffect(() => {
    ...
    const mockData = [
      { id: 1, provider: 'FedEx', Fee: 15000, duration: '2-3일', description: '일반 배송' },
      { id: 2, provider: 'DHL', Fee: 25000, duration: '1-2일', description: '특급 배송' },
      { id: 3, provider: 'UPS', Fee: 35000, duration: '3일', description: '당일 배송' },
      { id: 3, provider: 'TNT Express', Fee: 40000, duration: '7일', description: '특수 배송' },
    ];
  ...
  const handleAddService = (e) => {
    ...
    setNewService({ provider: '', Fee: '', duration: '', description: '' });
  };
  ...
  const handleFeeChange = (e, setFunction, currentValue) => {
    ...
    if (value === '' || /^\d*$/.test(value)) {
      ...
      // 현재 객체 속성을 모두 유지하되, Fee 속성만 새 value로 덮어씀
      ...
      <form onSubmit={handleAddService} className="mb-8 p-4 bg-gray-50 rounded">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <input
            ...
            // newService.Fee 값이 존재하면 addCommas 함수를 사용해 쉼표를 추가한 문자열을 입력
            // 값이 없으면 빈 문자열을 ''으로 설정하여 아무 값도 표시되지 않음
            value={newService.Fee ? addCommas(newService.Fee) : ''}

            onChange={(e) => handleFeeChange(e, setNewService, newService)}
            required
          />
          <input
            type="text"
          ...
          <tbody>
            ...
            {filteredServices.map((service) => (
              ...
              <tr key={service.id} className="border-b text-center">
                ...
                <td className="px-6 py-4">{addCommas(service.Fee)}원</td>
          ...
          <div className="bg-white p-6 rounded-lg w-96">
            ...
            <form onSubmit={handleUpdateService}>
              <div className="space-y-4">
                {['provider', 'Fee', 'duration', 'description'].map((field) => (
                  <div key={field}>
                    ...
                    <input
                      ...
                      // Fee 필드인 경우 addCommas 함수로 숫자에 쉼표를 추가
                      // 그 외의 필드에서는 editingService[field] 값을 그대로 사용
                      value={field === 'Fee' ? addCommas(editingService[field]) : editingService[field]}

                      onChange={(e) => field === 'Fee'
                        ? handleFeeChange(e, setEditingService, editingService)
...
export default AdminDeliveryFeeComponents;